...
const RolesView: React.FC<RolesView> = ({ roles, setRoles, onNavigate }) => {
  ...
  const filteredRoles = roles.filter(c => {
    ...
    const matchesStatus = statusFilter === 'Active' || c.active === true;

    return matchesSearch && matchesStatus;
  });

  const handleStatusChange = (id: string, newStatus: any, message: string) => {
    setRoles(prev => prev.map(c => c.id === id ? { ...c, active: newStatus } : c));
    showToast(message, newStatus === 'false' ? 'error' : 'success');
  };
  ...
  const confirmDelete = () => {
    if (roleToDelete) {
      setRoles(prev => prev.filter(c => c.id !== roleToDelete.id));
      showToast(`${roleToDelete.name} has been permanently deleted.`, 'success');
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden animate-fade-in relative">
      ...
      <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button 
            onClick={() => onNavigate(ViewState.ADD_ROLE)}
                ...
                <tr key={role.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                        <div className="text-sm font-bold text-slate-800">{role.name}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-slate-800">{role.description}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                    {role.code}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm font-bold ${getScoreColor(role.active)}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-2 opacity-100">
                      ...
                      <button 
                        onClick={() => handleStatusChange(role.id, 'Rejected', `Rejected ${role.name}`)}
                      ...
                      <button 
                         onClick={() => showToast(`Email draft opened for ${role.name}`, 'info')}
                      ...
                      <div className="relative action-menu-container">
                        <button 
                          onClick={() => setActiveActionMenuId(activeActionMenuId === role.id ? null : role.id)}
                          className={`p-1.5 rounded-md transition-colors ${activeActionMenuId === role.id ? 'bg-slate-100 text-slate-700' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100'}`}
                        >
                        ...
                        {activeActionMenuId === role.id && (
                          <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-slate-100 z-50 animate-fade-in text-left overflow-hidden">
                             ...
                             <button 
                                onClick={() => initiateDelete(role.id, role.name)} 
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100">
            <div className="p-6 text-center">
              ...
              <p className="text-slate-500 text-sm mb-6">
                Are you sure you want to remove <span className="font-semibold text-slate-700">{roleToDelete.name}</span>? 
